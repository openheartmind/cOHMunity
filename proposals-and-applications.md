...
At RSAA25, we conducted a live pilot test that produced relative valuations for diverse contributions to the RSAA conference, spanning presentations, financial sponsorship, facilitation, and behind-the-scenes efforts. Since then, a full deployment at a community arts gathering demonstrated the mechanism's capacity to coordinate and recognise the full range of contributions to a multi-day community event. Together, these experiments demonstrate that the same mechanism can value all forms of contribution — labour, capital, and ideas — across diverse communities, using interoperable metrics tied to collective values. We now aim to take the next step: demonstrating interoperability in practice across a series of workshops at RSAA26, RSAfrica26, and RSLA26.

Each workshop will begin with a brief introduction to the mechanism and key findings. Next, we'll use our prototype web-app to nominate and approve contributions for review, then value them together using a simple pairwise choice between randomly assigned contributions. The app will aggregate these votes into a relative value score for each contribution and a total for each contributor. Crucially, each workshop will include both local contributions — such as presentations — and global contributions — such as the common website infrastructure — enabling exploration of the mechanism's interoperability across contexts. This will demonstrate a key property of the mechanism: that recognition metrics can accumulate across communities who adopt the same evaluation dimensions, incentivising contributors to make their outputs openly available.

We'll close by celebrating the key contributors who made each conference happen, and reflecting on the process and results. Following the workshops, we will compare datasets, analyse findings, and aim to publish as a proof of concept for cross-community interoperability, with recognition for all participants and contributors through the same mechanism explored within.
